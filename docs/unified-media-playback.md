...
- Videos start muted in a fresh session. YouTube, Rumble, and Spotify use their own sound controls; no duplicate app mute overlay covers inline provider players. Watch-party followers retain their local volume control because provider controls are disabled for synchronized viewers. The session sound preference survives provider changes; an autoplay rejection may temporarily mute an individual player without changing that preference.
...
Real provider checks demonstrated YouTube muted playback, Rumble muted playback on an existing direct post URL, upload → Rumble → YouTube → upload handoffs, Spotify takeover and explicit video return, and autoplay recovery after audio releases focus without scrolling. Mixed-media checks used desktop Chrome and the in-app Chromium browser. Desktop Safari also autoplayed both Rumble and an uploaded video muted on fresh, existing direct post pages; the provider/native playback clocks advanced without pressing Play. Rumble playback was rechecked after the final loader fix and removal of the duplicate app mute control. The real post check caught a zero-height document-body edge case in the fixed app shell; the viewport calculation and regression test now cover it. Focused tests also ran on a physical iPhone: **34 tests in 5 suites passed** before the user asked to stop further device testing. That device run predates the final provider, preview, sound-fallback, and fullscreen lifecycle refinements; those changes were covered by the final simulator run.
...
- After the final viewport and Rumble load-error fixes: **25 focused tests passed** (viewport, coordinator, Rumble bridge), including provider bootstrap failure before readiness.
...
- Final production web build passed (`SENTRY_UPLOAD=false npx nuxt build`), and **all 34 routes hydrated cleanly**, including two existing Rumble posts, an uploaded-video post, and a profile feed. The first run caught a malformed Rumble load-error callback; its fix is covered by a behavioral regression and the clean rerun. Temporary validation pages/media and route-list additions were removed.
...
Home-feed uploaded-video autoplay passed in the authenticated local account. Additional smoke checks reached discovery/search, an existing public group, and bookmarks; that group/search sample had no playable video and bookmarks were empty. Check-in detail was gated to a personal account. Those checks do not establish playback coverage for these data-dependent surfaces.

Exercise media-bearing examples in profiles, groups, discovery/search, bookmarks, check-ins, and replies, plus quoted/duplicate posts, lightboxes, and watch-party local interruption/return. Repeat on desktop Safari, iPhone Safari, and native iOS. Verify PiP/fullscreen, background/foreground, headphones unplugged, audio-session interruptions, and unavailable/provider-blocked content. A simulator or fake provider is not evidence that these platform/provider cases passed.
