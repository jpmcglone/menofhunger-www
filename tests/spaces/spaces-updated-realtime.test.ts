...
describe('spaces updated realtime wiring (structural)', () => {
  ...
  it('renders lobby and preview spaces as cards with implied titles', async () => {
    ...
    expect(index).toMatch(/moh-divide/)
    expect(index).not.toMatch(/flex flex-col gap-3/)
    expect(index).not.toMatch(/Be the first/)
    ...
    expect(spacePage).toMatch(/computeSpaceSeo/)
    expect(spacePage).toMatch(/twitterCard/)
  ...
  it('re-joins the open space when spaces:updated flips isActive true', async () => {
    ...
    expect(spacePage).toMatch(/spaceReady\.value = true/)
    expect(spacePage).toMatch(/presenceStack/)
    expect(spacePage).not.toMatch(/You're the first/)
  ...
  it('locks the watch-party player to 16:9 with an overlaid local volume control', async () => {
    ...
    expect(player).toMatch(/muteViewerForAutoplay/)
    expect(player).toMatch(/Watch with the room/)
    expect(player).toMatch(/unlockViewerPlayback/)
    expect(player).toMatch(/viewerPlaybackUnlocked/)
