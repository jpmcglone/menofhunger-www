...
describe('StyledTextarea composition safety', () => {
  ...
  it('posts with Cmd-Enter in composer mode and sends with Enter in chat', () => {
    ...
    expect(cmd).toContain("'Mod-Enter': send")
    expect(cmd).toContain("'Shift-Enter': insertNewline")
    expect(cmd).not.toContain("'Shift-Enter': send")
    expect(cmd).toContain('Enter: insertNewline')
