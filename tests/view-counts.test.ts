...
describe('unique people + total views', () => {
  it('measures feed visibility against the middle scroller, not the window', () => {
    ...
    expect(tracker).toContain('requestedRoot.contains(el)')
  ...
  it('flushes guest views with a durable anon id even when cookies are blocked', () => {
    const tracker = readFromRepo('composables/usePostViewTracker.ts')
    ...
    expect(anon).toContain('clientMemoryAnonViewId')
    expect(anon).toContain('ensureClientAnonViewId')
    expect(tracker).toContain("anon_id: opts.anonId")
    expect(tracker).toContain("mohUnauthorized: 'ignore'")
    expect(tracker).toContain('keepalive: true')
    expect(tracker).toContain('if (!opts.isAuthed && !opts.anonId) return')
    expect(tracker).toContain('for (const id of ids) pendingPostIds.add(id)')
    expect(landing).toContain('observePost(post.id, el)')
    expect(postRow).toContain('trackViews: true')
    expect(feedRow).toContain(':track-views="false"')
