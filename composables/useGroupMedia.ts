...
export function useGroupMedia(
  ...
  const mediaFeed = useCursorFeed<GroupMediaItem>({
    stateKey: 'group-media-feed-local',
    stateMode: 'local',
    buildRequest: (cursor) => {
      ...
      return {
        path: `/groups/by-slug/${encodeURIComponent(slugRef.value)}/media`,
  ...
  function paramsKey() {
    return [slugRef.value, opts.sort?.value ?? 'new'].join('|')
  }
