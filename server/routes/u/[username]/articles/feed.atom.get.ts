/**
 * Per-Author Articles Atom Feed — GET /u/:username/articles/feed.atom
 */
import { buildArticleAtomFeed } from '../../../../utils/article-feed'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const res = await $fetch<{ data: FeedArticle[] }>(`${apiBase}/articles`, {
      query: { authorUsername: username, limit: 50, sort: 'new', includeRestricted: true, includeBody: '1' },
      timeout: 10_000,
    })
  ...
  const xml = buildArticleAtomFeed({
    feedUrl: `${site}/u/${u}/articles/feed.atom`,
    alternateUrl: `${site}/u/${u}/articles`,
