/**
 * Global Articles Atom Feed — GET /articles/feed.atom
 * Autodiscovered via <link rel="alternate" type="application/atom+xml"> in nuxt.config.ts.
 */
import { buildArticleAtomFeed } from '../../../server/utils/article-feed'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const res = await $fetch<{ data: FeedArticle[] }>(`${apiBase}/articles`, {
      query: { limit: 50, sort: 'new', includeRestricted: true, includeBody: '1' },
      timeout: 10_000,
    })
    articles = Array.isArray(res?.data) ? res.data : []
  } catch { /* return empty feed on API failure */ }

  const xml = buildArticleAtomFeed({
    feedUrl: `${site}/articles/feed.atom`,
