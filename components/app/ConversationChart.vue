<template>
  <div>
    <div class="flex h-20 items-end gap-1" role="group" aria-label="Daily conversation activity">
      <button
v-for="(day, index) in days" :key="day.date" type="button"
        ...
        :aria-label="`${day.date}: ${day.replies} replies, ${day.boosts ?? 0} boosts, ${day.reposts} reposts, ${day.coins} coins, ${day.branches} new branches`"
        ...
        <span class="w-full rounded-t-sm bg-emerald-500" :style="{ height: `${day.reposts / max * 52}px` }" />
        <span class="w-full bg-orange-500" :style="{ height: `${(day.boosts ?? 0) / max * 52}px` }" />
        <span class="w-full bg-sky-500" :style="{ height: `${day.replies / max * 52}px`, minHeight: '2px' }" />
        <span class="mt-1 h-1 w-full rounded-full" :class="day.coins ? 'bg-amber-400' : 'bg-transparent'" />
      </button>
    </div>
    <div class="mt-1 flex justify-between text-[10px] moh-text-muted"><span>{{ days[0]?.date.slice(5) }}</span><span>{{ days.at(-1)?.date.slice(5) }}</span></div>
    <div class="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs moh-text-muted" aria-live="polite">
      ...
      <span class="inline-flex items-center gap-1"><span class="h-1.5 w-1.5 rounded-full bg-orange-500" />{{ totals.boosts ?? 0 }} boosts</span>
      <span class="inline-flex items-center gap-1"><span class="h-1.5 w-1.5 rounded-full bg-emerald-500" />{{ totals.reposts }} reposts</span>
...
const max = computed(() => Math.max(1, ...props.days.map(d => d.replies + d.reposts + (d.boosts ?? 0))))
const totals = computed(() => active.value ?? props.days.reduce((a, d) => ({ replies: a.replies + d.replies, reposts: a.reposts + d.reposts, boosts: a.boosts + (d.boosts ?? 0), coins: a.coins + d.coins, branches: a.branches + d.branches }), { replies: 0, reposts: 0, boosts: 0, coins: 0, branches: 0 }))
</script>
