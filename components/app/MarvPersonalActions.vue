<template>
  <div class="space-y-4">
    ...
    <div v-if="!data && loading" class="space-y-3"><Skeleton height="5rem" /><Skeleton height="5rem" /></div>
    ...
    <div class="moh-divide">
      <article v-for="action in data" :key="action.id" class="py-4">
        ...
        <div class="mt-3 flex flex-wrap gap-2">
          <template v-if="action.status === 'pending'"><Button :label="action.kind === 'draft' ? 'Keep draft' : 'Apply'" :disabled="!!busy" :loading="busy === action.id" @click="decide(action.id, 'confirm')" /><Button label="Dismiss" text :disabled="!!busy" @click="decide(action.id, 'cancel')" /></template>
